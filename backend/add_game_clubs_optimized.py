...
def search_game_clubs_for_university(university: str, crawler: WeChatCrawler, db) -> int:
    ...
    # 只搜索"高校名称+游戏"
    search_query = f"{university} 游戏"
    logger.info(f"\n搜索关键词: {search_query}")

    try:
        # 搜索公众号（限制 5 个结果，避免太多无关的）
        ...
        logger.info(f"\n{university} 添加完成，共添加 {added_count} 个社团")
        return added_count
...
def main():
    ...
    try:
        ...
        if success_universities:
            logger.info(f"\n成功的高校 ({len(success_universities)}):")
            for uni_info in success_universities:
                logger.info(f"  - {uni_info}")
        else:
            logger.warning("\n未添加任何社团，可能原因：")
            logger.warning("  1. 未找到相关公众号")
            logger.warning("  2. 达到请求限制（每小时30次）")
            logger.warning("  3. Token/Cookie 已过期")
